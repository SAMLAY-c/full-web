...
export class OpenAIProvider {
  ...
  constructor(apiKey?: string, model?: string, baseURL?: string) {
    // 使用硅基流动API
    const useSiliconFlow = !!env.SILICONFLOW_API_KEY;

    this.client = new OpenAI({
      apiKey: apiKey || (useSiliconFlow ? env.SILICONFLOW_API_KEY : env.OPENAI_API_KEY),
      baseURL: baseURL || (useSiliconFlow ? env.AI_BASE_URL : undefined)
    });
    this.model = model || env.AI_DEFAULT_MODEL;
  }
